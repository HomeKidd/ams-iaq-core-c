...
#define SENSOR_POLL_PERIOD 10000  // reading interval
...
while(1){
        ...
        while (iaq_core_is_heating(iaq_core)){  // busy waiting until sensor is heating up
                printf("!!! WARNING !!! Sensor is in Heating up phase which takes about 5 minutes from startup.\n");
        ...
        if (iaq_core_get_results(iaq_core, &tvoc_value, &co2_value)){
            printf("iAQ Core C Sensor: %d TVOC, %d CO2\n", tvoc_value, co2_value);
                ...
                printf("!!! ERROR !!! iAQ Core C reading failed\n");
                // error happened
            switch (iaq_core->error_code)
                {
                    case IAQ_CORE_I2C_BUSY:
                                            printf("!!! ERROR !!! Sensor is BUSY...\n");
                                            break;
                    case IAQ_CORE_I2C_READ_FAILED:
                                            printf("!!! ERROR !!! I2C BUS read failed...\n");
                                            break;                                            
                    case IAQ_CORE_SENSOR_ERROR:
                                            printf("!!! ERROR !!! Sensor error, its might be broken...\n");
                                            break;                    
                    default:
                            printf(" ");
                }
...
void user_init(void)
{
    ...
    if (iaq_core) xTaskCreate(iaq_core_task, "iaq_core_task", 256, NULL, 2, NULL); // If iAQ Core is available start the user_task
}
